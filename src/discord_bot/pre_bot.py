...
# Make reg_agent importable when pre_bot.py is run directly from src/discord/
...
from pre_tournament.reg_agent import run_agent, _PAYMENTS_THREAD_PREFIX
from pre_tournament.reg_agent.step1_download import save_registration_csv
from pre_tournament.reg_agent import parse_and_store, load_all_parsed
from pre_tournament.setup_agent.setup_agent import run_setup_agent
from pre_tournament.pool_alch_agent.pool_alch_agent import run_pool_alch_agent, PoolAlchDeps
from pre_tournament.payment_agent.payment_agent import run_payment_agent
from pre_tournament.pool_alch_agent.state import load_state, deps_from_state
from pre_tournament.config import load_pre_config, PreConfig
...
class HemaTournamentPrepareBot(commands.Bot):
    config: PreConfig | None = None
    ...
    async def setup_hook(self) -> None:
        ...
        try:
            # agent_config.json is auto-discovered at src/shared/config/agent_config.json
            self.config = load_pre_config(user_config_path)
...
class GeneralCog(commands.Cog):
    ...
    def __init__(self, bot: HemaTournamentPrepareBot) -> None:
        self.bot = bot
...
class RegistrationCog(commands.Cog):
    ...
    async def _invoke_agent(
        ...
        if self.bot.config is None:
            user_config_path = os.environ.get("USER_CONFIG")
            try:
                self.bot.config = load_pre_config(user_config_path)
...
def run() -> None:
    ...
    acquire_instance_lock("/tmp/hema-pre-bot.lock")
    token = os.environ["DISCORD_TOKEN"]
    bot = HemaTournamentPrepareBot()
