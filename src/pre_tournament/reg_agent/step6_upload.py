...
from shared.config.tracing import observe
...
def upload_fencers(fencers: list[FencerRecord], config: PreConfig, sh: gspread.Spreadsheet):
...
def upload_discipline(discipline_code: str, fencers: list[FencerRecord], ratings: dict[int, dict[str, FencerRating]], config: PreConfig, sh: gspread.Spreadsheet):
...
def setup_output_sheet(config: PreConfig) -> str:
...
def remove_fencers_from_sheets(names: list[str], config: PreConfig) -> dict[str, list[str]]:
