...
from pre_tournament.msgs import read_msg
...
def _heal_parser(current_source: str, tb: str, html: str, current_version: int, config: PreConfig) -> tuple[Callable, str, int]:
...
def fetch_ratings(fencers: list[FencerRecord], config: PreConfig) -> tuple[dict[int, dict[str, FencerRating]], set[int]]:
