...
# Ensure both `pre_tournament.*` and bare `step*` imports resolve regardless of
# how this module is loaded (as a package or as a script).
_DIR = Path(__file__).parent              # src/pre_tournament/reg_agent/
_PKG = _DIR.parent                        # src/pre_tournament/
_SRC_ROOT = _PKG.parent                   # src/
for _p in (_SRC_ROOT, _DIR):
    if str(_p) not in sys.path:
        sys.path.insert(0, str(_p))

import discord
from shared.config.tracing import get_langfuse_client, observe
from pydantic_ai import Agent, RunContext
from pydantic_ai.exceptions import ModelHTTPError

langfuse = get_langfuse_client()

from pre_tournament.config import PreConfig, PreUserConfig, load_pre_config, save_pre_config
...
from pre_tournament.msgs import read_msg as _read_msg, render_msg as _render_msg
from pre_tournament.setup_agent.setup_agent import SHARED_MEMORY_PATH
...
from shared.config.tracing import enabled as _tracing_enabled
if _tracing_enabled:
...
def _bot_email(config: PreConfig) -> str:
...
async def tool_set_output_sheet(ctx: RunContext[AgentDeps], url: str) -> str:
    ...
    save_pre_config(
        PreUserConfig(
            ...
            registration_sheet_url=ctx.deps.config.registration_sheet_url,
...
async def tool_generate_social_media_list(ctx: RunContext[AgentDeps]) -> str:
    ...
    try:
        ...
        user_config_path = os.environ.get("USER_CONFIG")
        fresh_config = load_pre_config(user_config_path)
